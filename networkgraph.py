...
for node in node_info:
    G.add_node(node['name'], size=node['n'])
    if node['parent']:
        G.add_edge(node['parent'], node['name'])

# Draw the graph
pos = nx.spring_layout(G)
sizes = [G.nodes[node]['size'] for node in G.nodes]
nx.draw(G, pos, with_labels=True, node_size=sizes, node_color='skyblue', font_size=10, font_color='black', edge_color='gray')
plt.title('Leukemia Classifications')
plt.show()